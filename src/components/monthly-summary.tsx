...
export function MonthlySummary({ month, monthlySalary, userEconomy }: { month: Months[], monthlySalary?: number, userEconomy?: Array<{ date: string; value: number }> }) {
   ...
   return (
      <div className={`relative max-w-[300px] h-max flex flex-col items-center gap-4 w-full mt-10 p-4 rounded-lg bg-slate-800 transition-all max-lg:max-w-full max-lg:overflow-hidden ${shouldShowSummary ? 'max-h-96' : 'max-lg:max-h-14'}`}>
         ...
         <div className="flex flex-col items-center gap-4">
            ...
            <span>
               Poupou: <FormattedPrice price={savingsMonth(month[0].date, userEconomy)} style="normal" />
            </span>

            <span>
               Saldo Final: <FormattedPrice
                  price={(monthlySummary.earning ?? 0) - (monthlySummary.expense ?? 0) - (savingsMonth(month[0].date, userEconomy))}
                  style="profit"
               />
            </span>
         </div>
